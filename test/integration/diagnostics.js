...
describe(
  'integration: diagnostics_channel',
  ...
  () => {
    ...
    it('can time a call, which is the point of the tracing channel', async () => {
      ...
      try {
        await clientBus.invoke(
          call('Echo', { signature: 's', body: ['timed'] })
        );
      } finally {
      ...
      assert.ok(
        elapsed !== null,
        'start and end should share a context object'
      );
      assert.ok(
        elapsed >= 0 && elapsed < 5000,
        `implausible timing: ${elapsed}`
      );
    });
