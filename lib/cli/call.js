 ...
 *
 * `plainValues` and `returnBigInt` are the defaults now, and are written out
 * anyway: printing a reply is not the same job as consuming one, and a tool
 * whose entire output is the shape of a value should not have that shape
 * change underneath it because a default moved. `returnBigInt` in particular
 * is what keeps uint64:18446744073709551615 from printing as
 * 18446744073709552000.
 *
 * `variants: 'wrap'` is the one that is not a default: `describe()` prints a
 * variant as `variant u 501` and needs the signature to do it. This used to be
 * spelled `plainValues: false`, which got the signature by reading the
 * parser's internal tree -- the only way to ask, before there was a way to
 * ask. A Variant carries the same thing without the tree.
 */
