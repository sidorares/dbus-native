...
const DBusService2       = require ('./DBusService2')
const EventEmitter       = require ('events').EventEmitter
const stdDbusIfaces      = require ('./stdifaces')
const DBusObject2Libs    = require ('./DBusObject2Libs')
...
module.exports = function bus(conn, opts) {
    ...
    /**
     * Will store all DBusService2s that are created and exposed on the bus.<br>
     * It is used to present introspection data.
     * @type {Map<DBusObject2>}
    */
    ...
    this.handleDBusMessage = function (msg = mandatory()) {
        ...
        // First, we fetch the corresponding DBusService2 (and assign it to currObj to initiate traversing)
        ...
        while (!abord && pathComponents.length > 0) {
            ...
            if (! (currObj[currPathComponent] instanceof DBusObject2)) {
        ...
        if (! (currObj[msg.interface] instanceof DBusInterface2)) {
            // Reply with a proper DBus error
            let str = `No such interface '${msg.interface}' on object at path '${msg.path}'`
        ...
        */
        ...
        if (msg.type == constants.messageType.methodCall) {
            ...
            */
            ...
            // Here we have found the function, let's check the input argument's type match
            // if (!signature.doInputParamsMatch (iface._ifaceDesc.methods.get(methodName).input, msg.body)) {
            //     // Reply with a proper DBus error
            //     let str = `Arguments for method '${methodName}' don't match signature.`
            //     self.sendError (msg, 'org.freedesktop.DBus.Error.InvalidArgs', str)
            //     return
            // }

            // console.log ('input param match')
            // console.log ('/!\\ Params matching SKIPPED!')
            ...
            */
            ...
            .then( (ret) => {
                ...
                // If we have some return value form the function, build the 'reply.signature' and 'reply.body' field
                if (typeof ret !== 'undefined') {
                    // Get the output
                    output = iface._ifaceDesc.methods.get(methodName).output

                    // In case the 'output' field is an array but has only one element, unwrap it from the array
                    if (Array.isArray (output) && output.length === 1) {
                        output = output[0]
                    }

                    // If there is only one return type, deal with it
                    if (!Array.isArray (output)) {
                        let translatedType = utils.translateTypesBetweenAPIs (ret)
                        // console.log (inspect (translatedType, {colors: true}))

                        // We can take the first key because the output field was validated before and has only 1 key
                        reply.signature = output[Object.keys(output)[0]]
                        reply.body = translatedType

                        self.connection.message (reply)
                        return
                    }
                    else {
                        throw new Error ('Returning multiple values is not yet supported')
                        // console.log ('output: ' + inspect (ret, {colors: true}))
                        // let translatedType = utils.translateTypesBetweenAPIs (ret)
                        // console.log ('multiple translation: ' + inspect (translatedType, {colors: true}))
                        // throw new Error ('Multiple return values unsupported yet.')
                    }
    ...
    /**
     * Expose a DBusService2 to the bus.
     * @param {DBusService2} service The {@link DBusService2} object that we will expose on the bus
     ...
     */
     this.exposeService2 = function (service = mandatory(), serviceName = mandatory(), flag = utils.DBUS_NAME_FLAG_REPLACE_EXISTING | utils.DBUS_NAME_FLAG_DO_NOT_QUEUE) {
            ...
            if (retCode === utils.DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
                ...
                */
                ...
                /*
                    ...
                    - Find the signals
                    - listen to them
                    - convert them into DBus signal and emit them on the bus
                */
                function findAndRelaySignals(obj, path = '') {
                    ...
                    // First, find all signals on this object's interfaces and re-emit then into DBus signal
                    for (let ifaceName of ifacesNames) {
                        if (DEBUG) console.log ('\tiface: ' + ifaceName)
                        ...
                        for (let [signalName, signalType] of signalsDef) {
                            ...
                            obj[ifaceName].on (signalName, (...args) => {
                                ...
                                for (let typeSig of signalType.output) {
                                    let k = Object.keys (typeSig)[0] // Signature elems have only one key: their name
                                    signature += typeSig[k] // append the signature
                                }

                                console.log ('signature: ' + signature)
                                console.log ('body: ' + inspect (utils.translateTypesBetweenAPIs (...args)))
                                ...
                                self.sendSignal (path,
                                                 ...
                                                 signalName,
                                                 signature,
                                                 utils.translateTypesBetweenAPIs (...args)
                                                 )
                            })
                        }
                    }
                    ...
                    for (let childPath of childrenPath) {
                        ...
                        findAndRelaySignals (obj[childPath], path + '/' + childPath)
                    }
                }
                if (DEBUG) console.log ('Deal with object /')
                findAndRelaySignals (currObj)
