...
module.exports = function bus(conn, opts) {
     ...
     * @todo Check that the service DOES exist befoire returning it
     */
    this.getService2 = function (serviceName = mandatory()) {
        ...
        return new Promise ((resolve, reject) => {
            self.listNames ((err, names) => {
                if (err) reject (err)
                    ...
                    if (names.findIndex ( (name) => name === serviceName) !== -1)
                        // If the service exists, create a DBusService to represent it and return it
                        resolve (new DBusService2 (serviceName, self))
                    else {
                        /*
                            We tried to request a service that is not available on the bus, for the moment, we simply
                            return an error.
                            TODO: first check if the service is activatable, yes it, active it and return the service,
