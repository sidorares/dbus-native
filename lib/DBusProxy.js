						...
						*/
						...
						.then( () => {
							...
							this.bus.signals.on (mangledSignalName, (msg, signature_) => {
								...
								/*
									At the moment, we don't support returning several values, and PropertiesChanged does
									so we ignore it at the moment.
									It's lame, but this is still a WIP, we will solve this.
								*/
								if (signalName !== 'PropertiesChanged') {
									let translatedArgs = utils.fromOldtoNewAPI (msg[0], signature (signature_)[0])

									if (DEBUG) console.log ('translatedArgs:\n' + inspect (translatedArgs, {depth: 5}))
									parent[name].emit (signalName, translatedArgs)
								} else {
									// Will be removed soon, when we deal with multiple return values
