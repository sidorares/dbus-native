...
const headerSignature = parseSignature.single('(yv)');
  ...
  stream.on('readable', function() {
    ...
    while (1) {
      if (state == 0) {
        ...
        var unmarshalledHeader = messageBuffer.readArray(headerSignature, fieldsLength);
        ...
        for (var i = 0; i < unmarshalledHeader.length; ++i) {
          ...
          message[headerName] = unmarshalledHeader[i][1][1];
        }
...
module.exports.unmarshall = function unmarshall(buff) {
  ...
  for (var i = 0; i < headers.length; ++i) {
    var headerName = constants.headerTypeName[headers[i][0]];
    message[headerName] = headers[i][1][1];
  }
  msgBuf.align(3);
