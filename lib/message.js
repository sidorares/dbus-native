...
module.exports.write = function(message) {
    ...
    var bodyLength = message.body ? message.body.length : 0;
    var header = [ constants.endianness.le, type, flags, constants.protocolVersion, bodyLength, dbus.serial ];
    dbus.write(marshall('yyyyuu', header))
    var fields = [];
    ...
    dbus.write(marshall('a(yv)', fields));
    if (bodyLength > 0)
        dbus.write(message.body);
}
