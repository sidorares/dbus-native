...
module.exports.write = function(message) {
    ...
    var bodyBuff;
    if (message.body) {
        if (!Array.isArray(message.body) || message.body.length !== message.signature.length)
        {
            console.log(message.body.length, message.signature.length);
            throw new Error('message body does not match message signature. Body:' + message.body + ", signature:" + message.signature);
        }
    ...
    var fieldsBuff = marshall('a(yv)', [fields]);
