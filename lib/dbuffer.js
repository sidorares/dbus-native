...
// Wether or not to enable the new API
const NEW_API = true
...
DBusBuffer.prototype.readTree = function readTree(tree) {
	// console.log ('[READ TREE : ' + tree.type + ']')
	let ret
	let temp
	switch (tree.type) {
		case '(':
		...
		case 'r':
			...
			this.align(3);
			// ret = this.readStruct(tree.child)
		...
		case 'a':
			...
			// ret = this.readArray(tree.child[0], arrayBlobLength);
			// console.log ('\t' + inspect (ret, {colors: true, depth: 5}))
			// console.log ('That was an ARRAY')
			// break
			// return ret
		case 'v':
			return this.readVariant();
			// ret = this.readVariant();
			// break
		default:
			return this.readSimpleType(tree.type);
			// ret = this.readSimpleType(tree.type)
	}
...
DBusBuffer.prototype.readStruct = function readStruct(struct) {
	// console.log ('[READ STRUCT]')
	// console.log ('struct:\n' + inspect (struct, {colors: true, depth: 5}))
/* NEW API
    let obj = {}
...
//* OLD API
  ...
  // console.log ('result:\n' + inspect (result, {colors: true, depth: 5}))
  // console.log ('[/READ STRUCT]')
  return result;
//*/
}
...
DBusBuffer.prototype.readArray = function readArray(eleType, arrayBlobSize) {
	...
	// console.log ('type: ' + inspect (eleType, {solors: true, depth: 5}))
	// console.log ('size: ' + inspect (arrayBlobSize, {solors: true, depth: 5}))
	var result = []
	var key, value;
	var start = this.pos;
