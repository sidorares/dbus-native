 ...
 * @param {Object} objDesc The description object containing maps definition the DBus types for members
 * @param {Map<string, Object>} objDesc.methods A Map containing the signatures for functions
 * @param {Map<string, Object>} objDesc.properties A Map containing the signatures for properties
 * @param {Map<string, Object>} objDesc.signals A Map containing the signatures for signals
 *
 * @throws {module:Errors#InvalidNameError}
 */
class DBusInterface2 extends EventEmitter {
	constructor (ifaceName = mandatory(), objDesc = mandatory()) {
		super()
		...
		let copy = {}
		copy.methods = new Map (objDesc.methods)
		copy.signals = new Map (objDesc.signals)
		copy.properties = new Map (objDesc.properties)
 ...
 * @param {Object} objDesc - The object on which add the annotation data, it will be passed to the class's constructor
 ...
 */
function DBusMethod (objDesc = mandatory(), funcName = mandatory(), funcSignature = mandatory()) {
	...
	objDesc.methods.set (funcName, funcSignature)
}
 ...
 *                                 what is a signature-formatted object)
 */
function DBusProperty (objDesc = mandatory(), propName = mandatory(), propSignature = mandatory()) {
	// Create necessary, empty Maps in the object if they don't exist
	initObjDesc (objDesc)
	...
	// Add the introspection data to the introspection structure (possibly updating / overiding previous value)
	objDesc.properties.set (propName, propSignature)
}
 ...
 *                                 what is a signature-formatted object)
 */
function DBusSignal (objDesc = mandatory(), sigName = mandatory(), sigSignature = mandatory()) {
	// Create necessary, empty Maps in the object if they don't exist
	initObjDesc (objDesc)
	...
	// Add the introspection data to the introspection structure (possibly updating / overiding previous value)
	ifaceClass.signals.set (sigName, sigSignature)
}
 ...
 */
function initObjDesc (objDesc = mandatory()) {
	if (typeof objDesc.methods === 'undefined')
		objDesc.methods = new Map()

	if (typeof objDesc.properties === 'undefined')
		objDesc.properties = new Map()

	if (typeof objDesc.signals === 'undefined')
		objDesc.signals = new Map()
}
