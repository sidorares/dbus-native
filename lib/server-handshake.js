...
var exports = module.exports = function serverHandshake(stream, opts, cb) {
  ...
  readLine(stream, function(hello) {
    console.log('hello string: ', hello.toString()); // <-  AUTH EXTERNAL 31303030
    stream.write('OK eeab20b9d93db98226b95de9552b12d9\r\n');
    readLine(stream, function(line) {
      console.log('begin string: ', line.toString()); // <-  BEGIN
      cb(null);
    });
  });
};
...
var exports = module.exports = function serverHandshake(stream, opts, cb) {
  ...
  readLine(stream, function(hello) {
    ...
    //readLine(stream, function(auth1) {
    //  console.log(['AFTER AUTH LIST: ', auth1.toString()])
    //  stream.write('DATA ' + Buffer('org_freedesktop_general 642038150 b9ce247a275f427c8586e4c9de9bb951').toString('hex')+ '\r\n');
      //debugger;
    //  readLine(stream, function(dataRepl) {
        stream.write('OK 6f72675f667265656465736b746f705f67656e6572616c20353631303331333937206239636532343761323735663432376338353836653463396465396262393531\r\n');
        //readLine(stream, function(data) {
        //  console.log(['AFTER data: ', data.toString()])
        //  stream.write('OK e12a29dd7ffe3effac5eb95054123f80\r\n');
