const debug          = require ('debug')('dbus-native:stdifaces')
...
module.exports = function(msg, bus) {
    if (msg['interface'] === 'org.freedesktop.DBus.Introspectable' && msg.member === 'Introspect') {
        ...
        if (bus.exposedServices.has (msg.destination)) {
            ...
            paths.shift()
            //... and add the '/'
            if (paths[0] === '') // condition to avoid an empty elem when only '/' is asked for instrospection
                paths = ['/']
            else
                paths.unshift ('/')
        ...
        if (bus.exposedServices.has (msg.destination)) {
            ...
            // Let the new Properties class handle these calls
            return false
            ...
            // Traverse the object hierarchy of the DBusService and its DBusObjects (this is called folding, by the way)
            ...
            if (msg.member === 'Get' || msg.member === 'Set') {
                ...
                // If it's a SET method, set the new property and emit 'PropertiesChanged'
                    ...
                    if (data === undefined) {
                        ...
                        debug ('Could not parse data, Set operation aborted. (Data was:\n' + inspect (msgBody) + ')')
                ...
                let interfaceName
                let answerDict = {} // The object in which we will "push" our properties
                /*
                    'a{sv}' is the signature of the return type for 'GetAll'.
                    We build the tree here because we will need to it convert 'answerDict' form the new to the old API
                    (because the marshalling function still need the old API for now)
                */
                let tree = parseSignature ('a{sv}')[0]
                let targetIface // Will point to the interface whose properties we are interested in
                let descProperties // Will point ot the 'properties' array of the interface description (to loop over)

                // Check if we have supplied an interface name (prevent against malformed messages)
                if (msg.body === undefined || msg.body.length === 0) {
                    bus.sendError (msg, 'org.freedesktop.DBus.Error', 'No interface supplied to \'GetAll\'')
                    return true
                }

                // Now checks are ok, populate the variables
                interfaceName = msg.body[0]
                targetIface = targetObject[interfaceName]
                descProperties = targetIface._ifaceDesc.properties
                ...
                for (let [propertyName, propAccess] of descProperties) {
                    ...
                    if (accessMode !== 'write') {
                        ...
                        let variant = {
                            type: propType,
                            value: targetIface[propertyName]
                        }
                ...
                reply.body = [utils.fromNewToOldAPI (answerDict, tree)]
