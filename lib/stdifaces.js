...
module.exports = function(msg, bus) {
    console.log('STDINTARFACES MATHD CALL', msg);
    if (msg['interface'] === 'org.freedesktop.DBus.Introspectable' && msg.member === 'Introspect') {
        if (msg.path == '/')
            msg.path = '';

        console.log(JSON.stringify(bus.exportedObjects, null, 4));
        ...
        console.log(bus.exportedObjects);
        ...
        console.log(msg.body, interfaceName, obj);
        ...
        if (msg.member === 'Get' || msg.member === 'Set') {
            ...
            props = [];
            for (p in obj[interfaceName][0].properties) {
...
function interfaceToXML(iface) {
   ...
   var dumpArgs = function(argsSignature, argsNames, direction) {
       ...
       var args = parseSignature(argsSignature);
           ...
           argName = argsNames ? argsNames[num] : direction + num;
