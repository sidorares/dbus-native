...
module.exports = function(msg, bus) {
    ...
    if (msg['interface'] === 'org.freedesktop.DBus.Introspectable' && msg.member === 'Introspect') {
        ...
        // match input path (msg.path) against list of exported objects. For exacxt mach, dump nodes wuth interfaces
        // for "folder" match dump next level nodes

        // if we export "/", "/foobar", "/foo/bar", "/foo/baz" objects
        // and have introspect request for "/"
        // we need to respond "/" with interfaces and empty "foobar", "foo" nodes
        // if request is "/foo"
        // we respond "bar" and "baz"
        ...
        for (var path in bus.exportedObjects) {
           if (startsWith(path, msg.path)) {// objects path starts with requested
               obj = bus.exportedObjects[path];
               if (msg.path == path) // exact match
                   nodes[msg.path] = obj;
               else {
                   var prefixLength = ( msg.path == '/' ? 0 : msg.path.length);
                   if (path[prefixLength] != '/')
                       continue;
                   var localPath = path.substr(prefixLength);
