			...
			for (let iface of ifaces) {
				...
				if (utils.isValidIfaceName (name)) {
					...
					for (let signal of signals) {
						...
						console.log ('Will have to deal with signal ' + signalName)
						console.log ('The rule: ' + matchRule)
					...
					// Make the Javascript interface listen for the 'PropertiesChanged'
					parent[name].on ('PropertiesChanged', (...args) => {
						console.log ('\n')
						console.log ('[PROPERTIES CHANGED]')
						console.log (inspect (args, {colors: true, depth: 4}))
						console.log ('[END PROPERTIES CHANGED]')
					})
