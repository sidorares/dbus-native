...
var exports = module.exports = function auth(dbus, opts) {
    ...
    authMethods = (opts.authMethods && opts.authMethods.filter(function() { return true})) || ['EXTERNAL', 'DBUS_COOKIE_SHA1', 'ANONYMOUS'];
    tryAuth.call(this, authMethods, dbus, opts);
}

function tryAuth(methods, dbus, opts) {
    var authMethod = methods.shift();    
    console.log('TRYING ' + authMethod);
    ...
    function beginOrNextAuth() {
        binarystream
          ...
          .tap(function(vars) {
              ...
              console.log(ok);
              console.log(vars.reply.toString());
              if (ok && ok[1] === 'OK') {
    ...
    case 'DBUS_COOKIE_SHA1':
        ...
        binarystream
          ...
          .tap(function(vars) {
              ...
              // any random 16 bytesshould work, sha1(rnd) to make it simplier
