  ...
  function checkValidSignature(data) {
    if (data.length > 0xff) {
      throw new Error(
        'Data: ' +
          data +
          ' is too long for signature type (' +
          data.length +
          ' > 255)'
      );
    }
...
var makeLong = function(val, signed) {
  ...
  if (typeof val === 'number') {
    ...
    try {
      ...
      e.message =
        "Error converting number to 64bit integer '" + e.message + "'";
      throw e;
    }
  }
  if (typeof val === 'string' || val instanceof String) {
    ...
    try {
      var data = Long.fromString(val, !signed, radix);
    } catch (e) {
      e.message =
        "Error converting string to 64bit integer '" + e.message + "'";
      throw e;
    }
