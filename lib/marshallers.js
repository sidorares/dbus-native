...
var MakeSimpleMarshaller = function(signature) {
    ...
    switch(signature) {
        ...
        case 't':
            //UINT64
            marshaller.check = function(data) {
                // should be able to handle up to 53 bits in a javascript double number
                try {
                    checkInteger(data);
                    checkRange(0,0x1fffffffffffff,data); // since data is being passed as double we cannot claim the sign bit, so max is same as with signed int 64
                }
                catch (e) { e.message += " (NOTE: 64 Bit integers limited to 53 bits)"; throw e; }
            };
            marshaller.marshall = function(ps,data) {
                this.check(data);
                align(ps, 8);
        ...
        case 'x':
            ...
            marshaller.check = function(data) {
                ...
                try {
                    checkInteger(data);
                    checkRange(-0x1fffffffffffff,0x1fffffffffffff,data);
                }
                catch (e) { e.message += " (NOTE: 64 Bit integers limited to 53 bits)"; throw e; }
            };
            marshaller.marshall = function(ps,data) {
