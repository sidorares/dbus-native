    ...
    if (Array.isArray(root.interface)) {
      for (let i of root.interface) {
        ...
        if (iface !== null) {
          ...
          this.bus.addMatch(
            `type='signal',interface='${iface.$name}',path='${this.path}'`
          );
          for (let signal of iface.$signals) {
            ...
            this.bus.signals.on(event, (body, signature) => {
              if (signature !== signal.signature) {
                console.error(
                  `warning: got signature ${signature} for signal ${
                    iface.$name
                  }.${signal.name} (expected ${signal.signature})`
                );
                return;
              }
    ...
    for (let i = 0; i < args.length; ++i) {
      if (inSignatureTree[i].type === 'v') {
        if (args[i].constructor !== Variant) {
          throw new Error(
            `method call ${iface}.${member} expected a Variant() argument for arg ${i +
              1}`
          );
        }
