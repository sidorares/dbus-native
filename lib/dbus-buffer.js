...
function DBusBuffer(buffer, startPos, options, endianness) {
  ...
  // `ReturnLongjs` is an explicit request for the pre-2.0 shape, so on its own
  // it opts out of BigInt: now that `returnBigInt` defaults to true, reading
  // it as "both are set, BigInt wins" would leave a documented option doing
  // nothing at all. Asking for `returnBigInt: true` as well still wins.
  this.returnBigInt =
    this.options.returnBigInt === undefined
      ? !this.options.ReturnLongjs
      : this.options.returnBigInt !== false;
