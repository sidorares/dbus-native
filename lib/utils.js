...
function useless2 (acc, currVal) {
	let types = acc[0]
	let signature = acc[1]
	let newAcc = []

	// console.log ('FRomOldTONEW\nacc:' + inspect (acc, {depth: 4}))
	// console.log ('currVal: ' + inspect (currVal))

	/*
		If the fist element in the signature is a single type, return it
	*/
	if (singleTypes.includes (signature[0])) {
		console.log ('Single value: ' + inspect (currVal))

		// Push the new converted type (which is just itself for single values)
		types.push (currVal)
		// Discard the single-character representing the single type we just consummed
		signature = signature.substr(1)
		// Build new accumulator
		newAcc = [types, signature]
	...
	/*
		If the first element in the signature is 'a', then it's an array
	*/
	if (signature[0] === 'a') {
		console.log ('Array: ' + inspect (currVal))

		// Array of single types
		if (singleTypes.includes (signature[1])) {
			console.log ('\tof single types')
			// push the array of single types
			types.push (currVal)
		...
		throw new Error ('Can t parse arrays yet')
	}

	throw new TypeError ('Failed to parse data according to signature!')
}

// function fromOldtoNewAPI (val) {
function useless (val) {
	...
	*/
	if (Array.isArray (val) && !Array.isArray (val[0])) {
		console.log ('Array of single values')
	...
	/*
		A DICT entry
		It is represented as an array of array. Each array has 2 elements, the first being the key, and the second
		being the value
	*/
	if (Array.isArray (val) && val.every (e => Array.isArray (e) && e.length === 2)) {
		console.log ('DICT entry')
		let m = new Map (val) // we can build a map from an array of 2-array
