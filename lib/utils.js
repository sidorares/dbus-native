 ...
 * As it is very complex to dive into the marshallign and change it, and since I'm on a timeline (and since, afterall,
 * it works fairly well), the goal of this function is to translate the return types from the function that use the new
 * API (so return, Map, custom structures, etc) to the nested arrays-like types that is used on the marshalling
 * side.<br>
...
function translateTypesBetweenAPIs (retVal) {
	...
	*/
	...
	// Otherwise it must be a sigle type
	else {
		if (DEBUG) {
