...
function handleGetProperty(bus, msg, name, path) {
  ...
  if (
    !(property.access === ACCESS_READWRITE || property.access === ACCESS_READ)
  ) {
    bus.sendError(
      msg,
      INVALID_ARGS,
      `Property does not have read access: '${prop}'`
    );
  }
...
function handleSetProperty(bus, msg, name, path) {
  ...
  if (
    !(property.access === ACCESS_WRITE || property.access === ACCESS_READWRITE)
  ) {
    bus.sendError(
      msg,
      INVALID_ARGS,
      `Property does not have write access: '${prop}'`
    );
  }
  ...
  if (valueSignature !== property.signature) {
    bus.sendError(
      msg,
      INVALID_ARGS,
      `Cannot set property '${prop}' with signature '${valueSignature}' (expected '${
        properties[prop].signature
      }')`
    );
    return;
  }
...
module.exports = function(msg, bus) {
  let { path, member, destination, signature } = msg;
    ...
    if (m === member && method.inSignature === signature) {
      ...
      if (result === undefined) {
        ...
        throw new Error(
          `method ${
            iface.$name
          }.${m} expected to return multiple arguments in an array (signature: '${
            method.outSignature
          }')`
        );
      }
      ...
      if (method.outSignatureTree.length !== result.length) {
        throw new Error(
          `method ${
            iface.$name
          }.${m} returned the wrong number of arguments (got ${
            result.length
          } expected ${method.outSignatureTree.length}) for signature '${
            method.outSignature
          }'`
        );
      }
      ...
      for (let i = 0; i < result.length; ++i) {
        if (method.outSignatureTree[i].type === 'v') {
          ...
          body.push(
            variant.jsToMarshalFmt(method.outSignatureTree[i], result[i])[1]
          );
        }
      }
