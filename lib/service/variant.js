  ...
  if (signature.type === 'a') {
    ...
    if (signature.child[0].type === '{') {
      ...
      if (!Array.isArray(value)) {
        throw new Error(
          `expecting an array for signature '${signatureStr}' (got ${typeof value})`
        );
      }
      for (let v of value) {
        if (v.constructor === Variant) {
    ...
    if (value.length !== signature.child.length) {
      throw new Error(
        `expecting struct to have ${signature.child.length} members (got ${
          value.length
        } members)`
      );
    }
    ...
    for (let i = 0; i < value.length; ++i) {
      ...
      if (signature.child[i] === 'v') {
        if (v.constructor !== Variant) {
          throw new Error(
            `expected a Variant for struct member ${i + 1} (got ${v})`
          );
        }
        result.push(jsToMarshalFmt(v.signature, v.value));
      } else {
