...
function createBroker(opts = {}) {
  ...
  function accept(stream) {
    const conn = dbus.createConnection({
      ...
      // again, so every value has to survive the round trip. The convenience
      // shapes are all lossy in a way that matters to a router and to nobody
      // else, so it opts out of every one of them:
      //
      //   returnBigInt  a 64-bit integer read as a Number is lossy above 2^53,
      //                 and is then refused on the way out with "Number
      //                 outside range". A bus that quietly rounds someone's
      //                 payload would be worse.
      //   variants      'wrap' keeps the signature the sender wrote. Flattened
      //                 to a bare value it has to be re-inferred, and a
      //                 Variant('u', 9) is delivered as 'i' -- the receiver is
      //                 told a type its peer never sent.
      //   plainValues   a dict read as an object collapses duplicate keys. The
      //                 spec calls such a message corrupt and does not require
      //                 a receiver to reject it, but silently rewriting one in
      //                 transit is not a router's decision to make.
      //
      ...
      // -- see ROADMAP 4.5. Until then, reading exactly is what makes the
      // round trip faithful.
      returnBigInt: true,
      variants: 'wrap',
      plainValues: false
