...
module.exports.isPropSignatureObj = function isPropSignatureObj (obj) {
    ...
    // Then it must have exactly one key, which can be either 'readonly' or 'readwrite'
    objKeys = Object.keys (obj)
    if (objKeys.length !== 1 || (!objKeys.includes ('readonly') && !objKeys.includes ('readwrite'))) {
        console.error ('Signature must have exactly one keys: \'readonly\' or \'readwrite\'')
        return false
    }

    // Check that the field of 'readonly' or 'readwrite' is correctly formatted
    prop = objKeys.includes ('readonly') ? obj.readonly : obj.readwrite
    if (!isValidDBusType(prop)) {
        console.error ('Signature\'s \'readonly\' or \'readwrite\' contains non-conformant element(s)')
        console.dir (prop)
        return false
    }
...
module.exports.isSigSignatureObj = function isSigSignatureObj (obj) {
    ...
    return isValidSignatureElem (obj)
