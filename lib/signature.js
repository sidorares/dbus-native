...
parseSignature.single = function(signature) {
  if (signature.length == 1)
    return { type: signature[0], child: [] };

  var trees = parseSignature(signature);
  if (trees.length !== 1)
    throw new Error('Signature is not a single type');
  return trees[0];
};

parseSignature.fromTree = function(tree) {
  var res = '';
  for (var i=0; i < tree.length; ++i) {
    if (tree[i].child.length === 0)
      ...
      if (tree[i].type === 'a') {
        res += 'a' + parseSignature.fromTree(tree[i].child);
      } else {
        res += tree[i].type + parseSignature.fromTree(tree[i].child) + match[tree[i].type];
      }
    }
  }
  return res;
...
//console.log(tree);
//console.log(parseSignature.fromTree(tree))
