...
const debug   = require ('debug')('dbus-native:signature')
...
module.exports.valueFromTree = function valueFromTree (msgBody) {
    ...
    // If this is an array
        ...
        debug ('This is what will be returned:\n' + inspect (ret))
    ...
    // If this is a STRUCT
        ...
        for (let i in tree[0].child) {
            ...
            debug ('recursiveRet (before): ' + inspect (recursiveRet))

            recursiveRet.push (recA)

            debug ('recursiveRet (after): ' + inspect (recursiveRet))
        }
        ...
        */
        ...
        debug ('Unsupported complex type!')
        return undefined
    }
}
...
module.exports.DBUS_VARIANT   = DBUS_VARIANT   = 'v'
...
function isValidDBusType (type) {
    ...
    /*
        At this point, what remains is a succession of single types with some containers type in it, like: ssas.
        So chekk if first element is a validSingleType and then proceed to check the rest of the string
    */
    if (isValidSingleType(type[0]))
        return isValidDBusType(type.substr(1))
